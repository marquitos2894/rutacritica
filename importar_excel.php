...
if(isset($_FILES)){
		...
		foreach ($_FILES as $archivo) {	
			...
			//----------------------				
				...
				calcular_tiempos($iteraciones[0]['matriz'], 
								$matriz2,
								$iteraciones[0]['duracion_fin'], 
								$num_columnas);
				
				
				
				obtener_ruta_critica($iteraciones[0]['letras_ruta_critica'], 
									$iteraciones[0]['ruta_critica'], 
									$iteraciones[0]['matriz']);
					
				
				crear_html(	$iteraciones[0]['html_tabla'], 
							$iteraciones[0]['html_actividades'], 
							$iteraciones[0]['html_enlaces'],
							$matriz2,
							$iteraciones[0]
				...
				while($reducir){
					...
					if(count($reducibles)>0){
						...
						while($existe){
							...
							calcular_tiempos($matriz, 
											$matriz2,
											$iteracion['duracion_fin'], 
											$num_columnas);
							obtener_ruta_critica($iteracion['letras_ruta_critica'], 
												$iteracion['ruta_critica'], 
												$matriz);
							...
							if($existe){
								...
								crear_html(	$iteracion['html_tabla'], 
											$iteracion['html_actividades'], 
											$iteracion['html_enlaces'],									
											$matriz2,
											$iteracion
								);		
	...
	echo $html_script;
		...
		}//fin de  foreach($_FILES as $archivo)
					
}//fin de if(isset($_FILES))
...
function crear_html(&$scripttabla, &$scriptact, &$scripenlaces, $matriz2, $iteracion){
				...
				$scripenlaces .= "<svg height='".($num_filas * 150 - 50)."px' width='".($num_columnas * 200 - 50)."px'>";			
				foreach($matriz2 as $actividad => $valor){
					if($valor['nivel_columna']>=1){
						foreach($valor['predecesor'] as $key => $valor2){
